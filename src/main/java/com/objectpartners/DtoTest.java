...
public abstract class DtoTest<T> {
	...
	/**
	 * Creates an instance of {@link DtoTest} with ignore fields and additional
	 * custom mappers.
	 *
	 * @param customMappers
	 *            Any custom mappers for a given class type.
	 * @param ignoreFields
	 *            The getters which should be ignored (e.g., "getId" or "isActive").
	 */
	...
	private void callGetter(String fieldName, Method getter, T instance, Object expected)
		...
		if (getter.getReturnType().isPrimitive()) {
			...
			/*
			 * This is a normal object. The object passed in should be the exactly same
			 * object we get back.
			 */
...
//			return clazz.newInstance();
	...
	/**
	 * Tests all the getters and setters. Verifies that when a set method is called,
	 * that the get method returns the same thing. This will also use reflection to
	 * set the field if no setter exists (mainly used for user immutable entities
	 * but Hibernate normally populates).
	 *
	 * @throws Exception
