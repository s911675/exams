...
public class LambdaTest {
	...
	public static void peopleTest() {
		...
		List<String> names = people.stream()
				...
				.map(person -> person.getName().toUpperCase()).collect(toList());
	...
	public static void stringArraysTest() {
		...
		myList.stream().filter(s -> s.startsWith("c")).map(s -> s.toUpperCase()).forEach(System.out::println);

	}

	public static void intArraysTest() {
		Arrays.stream(new int[] { 1, 9, 5, 3 }).map(x -> x * 2).average().ifPresent(System.out::println);
	}
	...
	public static void main(String[] args) {
		...
		List<?> list = "asdfas".chars().mapToObj(c ->(char)c)
